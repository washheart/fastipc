...
DWORD startClientProcess(std::wstring clientExe, std::wstring serverName){
	STARTUPINFO sui;
	...
	// 生成命令行
	std::wstringstream wss; wss << clientExe << L" " << serverName;
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR szCmdLine, int iCmdShow){
	...
	if (server.create(serverName) != 0){
		MessageBox(NULL, L"创建IPC服务器失败", L"消息", 0);
		return -1;
	}
	else{
		if (startClientProcess(clientExee, serverName) == 0){// 创建子进程，失败后返回
			return -1;
		}
	}
